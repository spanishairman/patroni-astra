...
> [!TIP]
> Для работы стенда также потребуются файлы ![ansible.cfg](/files/ansible.cfg) и ![hosts.psqla](/files/staging/hosts.psqla)
...
Для реализации условий по созданию кластера, на серверах группы _psqlserver_ потребуется выполнить следующие действия:
...
##### Клиентское приложение для подключения к базе данных

В качестве клиента базы данных используется _XMPP_ сервер _Ejabberd_. Подробное описание развёртывания отказоустойчивого экземпляра службы обмена мгновенными 
сообщениями можно найти [здесь](https://github.com/spanishairman/ejabberd-postgresql-debian).

Для текущей статьи описание плейбука можно посмотреть ![здесь](/files/play/psqla/7.web.yml).

##### Заключение

Для воссоздания работы стенда потребуется создать два образа - один для виртуальных машин под управлением _Astra Linux_, второй - для _Debian_.
Соответствующая инструкция находится [здесь](https://github.com/spanishairman/vagrant).
